...
- [Work flow của RNN (Không code)](#work-flow-của-rnn-không-code)
---
# Introduction
```bash
- RNN (Recurrent Neural Network) Là một kiến trúc mạng neural dùng để xử lý dữ liệu tuần tự (sequential data). 
- RNN hoạt động dựa trên ý tưởng: thông tin ở bước t−1 sẽ được kết hợp với đầu vào ở bước t để tạo ra trạng thái mới.
...
| ...                      |
```
**Tokenization: chuyển câu thành token**
```bash
I love deep learning -> [I, love, deep, learning]
```
**Vocabulary: tạo từ điển**
```bash
| Word     | ID |
| -------- | -- |
| I        | 1  |
...
-> Vocabulary size: V = 8
```
**Tạo Training Samples: RNN học theo sequence → next token**
```bash
I love deep learning
