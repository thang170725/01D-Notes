...
- [Giải tích](#giải-tích)
  ...
  - [Chain Rule (Quy tắc dây chuyền là quy tắc dùng để tính đạo hàm của một hàm nằm bên trong một hàm khác)](#chain-rule-quy-tắc-dây-chuyền-là-quy-tắc-dùng-để-tính-đạo-hàm-của-một-hàm-nằm-bên-trong-một-hàm-khác)
- [Xác suất thống kê](#xác-suất-thống-kê)
  ...
  - [KL Divergence (Kullback–Leibler Divergence trả lời câu hỏi Phân phối Q khác phân phối P bao nhiêu?)](#kl-divergence-kullbackleibler-divergence-trả-lời-câu-hỏi-phân-phối-q-khác-phân-phối-p-bao-nhiêu)
  ...
  - [Matrix (Ma trận)](#matrix-ma-trận)
    ...
    - [Matrix Norms (dùng để đo “độ lớn” của vector hoặc ma trận)](#matrix-norms-dùng-để-đo-độ-lớn-của-vector-hoặc-ma-trận)
      - [Frobenius norm](#frobenius-norm)
      - [Column norm (đo độ lớn theo từng cột)](#column-norm-đo-độ-lớn-theo-từng-cột)
      - [Infinity norm (chuẩn vô cực)](#infinity-norm-chuẩn-vô-cực)
      - [Spectral norm (chuẩn quang phổ - đo “mức phóng đại lớn nhất” của matrix)](#spectral-norm-chuẩn-quang-phổ---đo-mức-phóng-đại-lớn-nhất-của-matrix)
...
```
## Jaccard (Jaccard Similarity là một chỉ số đo độ giống nhau giữa hai tập hợp)
**Formula**
```bash
Jaccard = (Số phần tử chung) / (Tổng số phần tử khác nhau) # Hay viết toán học: J(A, B) = |A ∩ B| / |A ∪ B|
...
Giao: {"apple", "banana"} -> Có 2 phần tử
Hợp: {"apple", "banana", "cat", "dog"} -> Có 4 phần tử
...
**Ex**
```bash
y=(3x+2)**2 # Ở đây có 2 hàm lồng nhau: x -> 3x + 2 -> (3x + 2)² -> y
...
Chain Rule nói rằng: dy/dx = dy/du * du/dx = 2u×3 = 6(3x+2)
```
# Xác suất thống kê
...
```bash
Nói đơn giản: 
    “Ban đầu mình nghĩ khả năng xảy ra là bao nhiêu, rồi sau khi thấy bằng chứng mới thì nên tin lại như thế nào?”
...
```
## KL Divergence (Kullback–Leibler Divergence trả lời câu hỏi Phân phối Q khác phân phối P bao nhiêu?)
```bash
...
1. Nhưng KL không phải khoảng cách thông thường
...
```bash
Dùng để:
    - biến đổi không gian. Nó nhận một vector đầu vào và biến đổi nó.
```
**Ex: vật lý**
```bash
...
- V: đặc trưng ẩn
```
### Matrix Norms (dùng để đo “độ lớn” của vector hoặc ma trận)
```bash
Norm = cách đo “kích thước / độ lớn / mức độ mạnh” của vector hoặc matrix
```
#### Frobenius norm
#### Column norm (đo độ lớn theo từng cột)
**Ex**
```bash
A = [[1  4], [2  5], [3  6]]
