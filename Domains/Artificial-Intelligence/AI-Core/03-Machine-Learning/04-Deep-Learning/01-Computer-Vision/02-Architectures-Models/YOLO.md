...
└───────────────────────────────────────────────────────────────────────────────────────────────┼┼┼┘
                                                                                                ...
                                                                                                ▼▼▼
┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
│ 3. HEAD - PHẦN TRÍCH XUẤT KẾT QUẢ DỰ ĐOÁN (Lớp 22)                                               │
│                                                                                                  │
│  ├── [Lớp 22] Lớp Đuôi Detect() ──► Tiếp nhận đồng thời cả 3 đầu ra song song từ Neck:           │
│  │    ├─ Nhánh 1 (Từ lớp 15): [128 x 128 x 128] ──► Tính toán ma trận ──► 128x128 = 16384 vị trí │
│  │    ├─ Nhánh 2 (Từ lớp 18): [64 x 64 x 256]   ──► Tính toán ma trận ──► 64x64   = 4096 vị trí  │
│  │    └─ Nhánh 3 (Từ lớp 21): [32 x 32 x 512]   ──► Tính toán ma trận ──► 32x32   = 1024 vị trí  │
│  │                                                                                               │
│  ├── Thuật toán tích hợp bên trong lớp dự đoán (Anchor-Free):                                    │
│  │    ├─ Phẳng hóa (Flatten) và gộp toàn bộ các vị trí: 16384 + 4096 + 1024 = 21,504 Hộp bao     │
│  │    └─ Áp dụng DFL (Distribution Focal Loss): Chuyển đổi các kênh đặc trưng tọa độ             │
│  │       từ dạng phân phối xác suất liên tục thành 4 giá trị biên cứng [x, y, w, h].             │
└──────────────────────────────────────────────────────────────────────────────────────────────────┘
      │
      ▼
KẾT QUẢ ĐẦU RA CUỐI CÙNG (Final Output Shape) ──► Matrix kích thước [21504 x (4 + 1 + Số_Lớp)]
  │
  ├── 21,504 dòng: Đại diện cho tất cả các vùng không gian mà mô hình quét và nghi ngờ có vật thể.
  └── Mỗi dòng chứa các cột thông tin cụ thể:
        ├── [Cột 0 -> 3] : 4 giá trị tọa độ Box (x_center, y_center, width, height) đã qua DFL.
        ├── [Cột 4]      : 1 giá trị Objectness Score (Độ tự tin/tin cậy xem có vật thể hay không).
        └── [Cột 5 -> N] : Các giá trị Class Probability (Xác suất vật thể thuộc về nhãn phân loại).
```
