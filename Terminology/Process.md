...
- [Computer Network (mạng máy tính)](#computer-network-mạng-máy-tính)
  ...
  - [gRPC (Google Remote Procedure Call là một framework/protocol dùng để cho các service giao tiếp với nhau qua mạng)](#grpc-google-remote-procedure-call-là-một-frameworkprotocol-dùng-để-cho-các-service-giao-tiếp-với-nhau-qua-mạng)
    - [Ask](#ask)
      - [gRPC khác REST API như thế nào?](#grpc-khác-rest-api-như-thế-nào)
      - [Tại sao phải dùng .proto?](#tại-sao-phải-dùng-proto)
      - [gRPC rất mạnh ở Streaming](#grpc-rất-mạnh-ở-streaming)
      - [gRPC thường được dùng ở đâu?](#grpc-thường-được-dùng-ở-đâu)
      - [Tại sao không dùng REST hết?](#tại-sao-không-dùng-rest-hết)
- [Subdomain](#subdomain)
...
## gRPC (Google Remote Procedure Call là một framework/protocol dùng để cho các service giao tiếp với nhau qua mạng)
```bash
...
API Server muốn lấy thông tin user.
   - Nếu dùng REST: GET /users/123
   - Nếu dùng gRPC, bạn có thể định nghĩa một function: GetUser(user_id)
      Sau đó API Server gọi: user = user_service.GetUser(user_id=123)
...
```
### Ask
#### gRPC khác REST API như thế nào?
```bash
REST thường:
   Client
      │
      ...
      ▼
   Server
   -> Response thường là JSON: {"id": 123, "name": "Thang"}
...
gRPC thường:
   Client
      ...
      ▼
   Server
   -> Dữ liệu thường được serialize bằng: Protocol Buffers (Protobuf)
```
#### Tại sao phải dùng .proto?
```bash
Đây là điểm khác REST rất rõ. Bạn có thể coi .proto là hợp đồng (contract) giữa hai service.
```
**Ex**
```python
syntax = "proto3";
...
```
#### gRPC rất mạnh ở Streaming
```bash
REST thường kiểu: Request -> Response

gRPC hỗ trợ nhiều kiểu: Unary
...
```
#### gRPC thường được dùng ở đâu?
```bash
Đặc biệt là: Microservices
...
Payment Service
```
#### Tại sao không dùng REST hết?
```bash
REST hoàn toàn có thể dùng cho microservices.
   Nhưng khi hệ thống lớn: Service A -> Service B -> Service C -> Service D -> có rất nhiều request nội bộ.
