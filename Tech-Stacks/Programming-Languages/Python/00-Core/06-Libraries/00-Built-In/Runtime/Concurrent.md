- [Concurrent Introduction (dùng để thực hiện lập trình đồng thời (concurrency), giúp chạy nhiều tác vụ cùng lúc mà không cần tự quản lý thread hay process)](#concurrent-introduction-dùng-để-thực-hiện-lập-trình-đồng-thời-concurrency-giúp-chạy-nhiều-tác-vụ-cùng-lúc-mà-không-cần-tự-quản-lý-thread-hay-process)
- [futures](#futures)
...
# futures
## ThreadPoolExecutor (Dùng khi công việc chờ I/O)
```bash
Dùng khi:
    - Đọc ảnh (cv2.imread)
    - Đọc file
...
with ThreadPoolExecutor(max_workers=8) as executor:
    images = list(executor.map(cv2.imread, files))

# Nếu ổ SSD đủ nhanh thì việc đọc nhiều ảnh có thể nhanh hơn đọc tuần tự.
```
## ProcessPoolExecutor (Dùng khi công việc nặng về CPU)
**Ex**
```python
for image in images:
    heavy_algorithm(image)

# thì có thể đổi thành
