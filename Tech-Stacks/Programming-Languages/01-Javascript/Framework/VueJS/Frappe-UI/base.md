...
Framework nền tảng: Vue 3
CSS: Tailwind CSS

Ví dụ một component Frappe UI:
...
Bạn có thể thấy cú pháp này chính là cú pháp của Vue 3 (<script setup>, <template>).

Frappe UI và Frappe Framework khác nhau thế nào?

Nhiều người mới học hay nhầm hai khái niệm này.

Frappe Framework	Frappe UI
Framework full-stack	Thư viện UI
Backend + Frontend	Chỉ Frontend
Chủ yếu dùng Python (backend) và JavaScript	JavaScript / Vue 3
Có ORM, API, Authentication...	Có Button, Dialog, Form, Data Fetching...

Frappe Framework là một framework web full-stack được viết chủ yếu bằng Python (backend) kết hợp JavaScript cho phần frontend.
...
Frappe UI
      ...
      ├── Components
      └── Utilities
Frappe UI thường dùng để làm gì?

Frappe UI cung cấp sẵn các thành phần như:
...
Có thể dùng Frappe UI mà không cần Frappe Framework không?

Có.

Vì Frappe UI là một package npm, bạn có thể cài vào bất kỳ dự án Vue 3 nào:
...
Mặc dù được thiết kế để hoạt động tốt với Frappe Framework, nó vẫn có thể dùng trong một ứng dụng Vue độc lập.

Tóm tắt
Frappe UI → thư viện UI cho Vue 3, viết bằng JavaScript/TypeScript.
Frappe Framework → framework full-stack, backend chủ yếu bằng Python, frontend sử dụng JavaScript.
Nếu bạn đang phân loại tài liệu theo công nghệ, mình sẽ đặt:
Vue.js → Framework Frontend
Frappe UI → UI Library (thuộc hệ sinh thái Vue)
Frappe Framework → Python Web Framework (Full-stack).
```bash
